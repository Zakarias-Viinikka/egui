...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let visual = match argb_visual {
        Some(v) => v,
        None => {
            eprintln!("popup_demo: no 32-bit ARGB visual found, using root visual");
            screen.root_visual
        }
    };
    let depth: u8 = if argb_visual.is_some() { 32 } else { screen.root_depth };
    eprintln!("popup_demo: using depth {} visual {:#x}", depth, visual);
    ...
    loop {
        while let Some(event) = conn.poll_for_event()? {
            match event {
                Event::Expose(_) => match put_buffer(&conn, win, W, H, depth, &buf) {
                    Ok(()) => {
                        let _ = conn.sync();
                        drawn = true;
                    }
                    Err(e) => eprintln!("put_buffer failed: {:?}", e),
                },
                Event::ButtonPress(_) => {
        ...
        if !drawn {
            match put_buffer(&conn, win, W, H, depth, &buf) {
                Ok(()) => {
                    let _ = conn.sync();
                    drawn = true;
                }
                Err(e) => {
                    eprintln!("put_buffer failed (initial): {:?}", e);
                    drawn = true;
                }
            }
        }
...
fn render_content(w: u32, h: u32, text: &str) -> Vec<u32> {
    ...
    let bg = (0xE6u8, 0xE6u8, 0xE6u8);
    let fg = (0x18u8, 0x18u8, 0x18u8);

    // 1. draw text into an RGB buffer (alpha 1 where background, blended where text)
    let mut rgb: Vec<(u8, u8, u8)> = vec![bg; (w * h) as usize];
            ...
            for (m, bmp, gx) in glyphs {
                ...
                for row in 0..m.height {
                    for col in 0..m.width {
                        ...
                        let (br, bc_g, bb) = rgb[idx];
                        let a = boost_curve(cov as u32) as f32 / 255.0;
                        let nr = (fg.0 as f32 * a + br as f32 * (1.0 - a)).round() as u8;
                        let ng = (fg.1 as f32 * a + bc_g as f32 * (1.0 - a)).round() as u8;
                        let nb = (fg.2 as f32 * a + bb as f32 * (1.0 - a)).round() as u8;
                        rgb[idx] = (nr, ng, nb);
                    }
                }
            }
    ...
    // 2. multiply RGB by rounded-rect coverage, output premultiplied ARGB
    let mut buf = vec![0u32; (w * h) as usize];
    for py in 0..h {
        for px in 0..w {
            let (rr, gg, bb) = rgb[(py * w + px) as usize];
            let cov = rounded_alpha(px as f32 + 0.5, py as f32 + 0.5, wf, hf, r);
            let ai = (cov * 255.0).round() as u32;
            let rp = (rr as u32 * ai) / 255;
            let gp = (gg as u32 * ai) / 255;
            let bp = (bb as u32 * ai) / 255;
...
/// Coverage of a pixel inside a rounded rectangle: 0 outside, 1 well inside,
/// partial on the AA ring.
...
fn put_buffer<C: Connection>(
    ...
    // X11 ARGB32 on little-endian: bytes are B, G, R, A per pixel
