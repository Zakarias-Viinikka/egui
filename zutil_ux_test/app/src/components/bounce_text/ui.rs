...
const MAX_SCALE_BOOST: f32 = 0.18;
...
pub fn bounce_text_ui(ui: &egui::Ui, state: &BounceTextState) -> bool {
    ...
    // snappier rise, slower settle
    let warped = t.powf(0.7);
    let lift = (std::f32::consts::PI * warped).sin();

    let alpha = 1.0 - t;

    let scale = 1.0 + MAX_SCALE_BOOST * lift;
    let y_off = -MAX_LIFT * lift;

    let layer_id = egui::LayerId::new(egui::Order::Foreground, state.id);
    let painter = ui.ctx().layer_painter(layer_id);

    let scaled_size = state.size * scale;
    let rect = egui::Rect::from_center_size(state.center + egui::vec2(0.0, y_off), scaled_size);

    painter.rect_filled(rect, CORNER_RADIUS, with_alpha(BG_COLOR, alpha));

    let wrap = scaled_size.x - PADDING * 2.0 * scale;
